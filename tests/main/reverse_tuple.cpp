...
SCENARIO("Reverse Tuple Size, Alignment, and Offset") {
  static_assert(sizeof(reverse_tuple<>) == 1);
  static_assert(alignof(reverse_tuple<>) == 1);

  static_assert(sizeof(reverse_tuple<uint8, uint32>) == 8);
  static_assert(alignof(reverse_tuple<uint8, uint32>) == 4);
  //
  static_assert(sizeof(reverse_tuple<uint8, uint8, uint32>) == 8);
  static_assert(alignof(reverse_tuple<uint8, uint8, uint32>) == 4);
  ...
  types::for_each([]<typename type> {
    {
      ...
      using tuple_type = reverse_tuple<type>;
    ...
    types::for_each([]<typename type2> {
      ...
      types::for_each([]<typename type3> {
        ...
        using tuple_type = reverse_tuple<type, type2, type3>;
...
SCENARIO("Reverse Tuple Element Access") {
  { reverse_tuple<> x{}; }

  SUBCASE("Irreducible Types") {
    using tuple_type = reverse_tuple<int>;
    //
    tuple_type x{1};
  ...
  SUBCASE("Const LValue References") {
    using tuple_type = reverse_tuple<const int&>;
    int data = 1;
    tuple_type x{data};
...
SCENARIO("Reverse Tuple Constructors and Assignments") {
  using tuple_type = reverse_tuple<int, string, float>;
  ...
  SUBCASE("Conversions and Assignments from Convertible Tuples") {
    using other_tuple_type = reverse_tuple<size_t, czstring, double>;
...
SCENARIO("Reverse Tuple Class Template Argument Deduction") {
  int data = 1;
  const int cdata = 1;
  reverse_tuple x{data,  'c',    1.23f,     3.14,       "Test",
...
SCENARIO("Reverse Tuple Structured Bindings") {
  int data = 1;
  // int& test = move(data); // This is not allowed.
  reverse_tuple v{data, ref(data), cref(data)};
  // meta::print_type(v);
...
SCENARIO("Reverse Tuple Swapping") {
  using tuple_type = reverse_tuple<int, char, float>;
...
SCENARIO("Reverse Tuple Total Ordering") {
  using tuple_type = reverse_tuple<int, char, float>;
...
// SCENARIO("Reverse Tuple Dynamic Memory Layout") {}
...
SCENARIO("Reverse Tuple Preservation of Type Traits") {
  static_assert(is_standard_layout_v<reverse_tuple<>>);

  using tuples = type_list<reverse_tuple<int, string>,       //
                           reverse_tuple<uint8, uint64>,     //
                           reverse_tuple<czstring, string>,  //
                           reverse_tuple<int, float>>;
...
// SCENARIO("Constexpr Reverse Tuple") {}

SCENARIO("Reverse Tuple Type Logs") {
...

  {
    ...
    {
      reverse_tuple<log_int, log_int, log_string> x{1, 2, "Test"};
      ...
      reverse_tuple<log_int&, const log_int&, log_string&> y{x};
      ...
      // reverse_tuple<log_int, log_int, log_string> z{
      //     move(value<0>(y)), move(value<1>(y)), move(value<2>(y))};
      reverse_tuple<log_int, log_int, log_string> z{std::move(y)};
      // reverse_tuple<log_int, log_int, log_string> z{};
      // z = std::move(y);
      ...
      reverse_tuple<log_int, log_int, log_string> v{10, 20, "test"};
      ...
      // y = v;
      reverse_tuple<log_int&, const log_int&, log_string&> w{v};
